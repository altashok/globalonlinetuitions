<header>

<div class="mob-social-header hidden-md hidden-lg">
  <div class="container">
    <!--<div class="aff-no col-lg-5 col-md-5 col-sm-5 col-xs-5">
    Affiliation No : 123456
    </div>
    <div class="downloads col-lg-7 col-md-7 col-sm-7 col-xs-7">
    <a href="https://gaitview.com/gvt/login.html" target="_blank">Parents Login</a>
    </div>-->
    <div class="col-sm-4 col-xs-4">
      <div class="folow-us">
        <a href="https://www.facebook.com/globalonlinetuitions" target="_blank"><img src="images/facebook.png" alt="social"/></a>
      </div>
    </div>
    <div class="col-sm-8 col-xs-8 no-pad-menu">
      <div class="conatct-detail">
        <!--<div class="downloads">
        <a href="downloads.html">Downloads</a>
        </div>-->
        <div class="phone-num">
        +91 97157 53588, +91 86829 01099
        </div>
      </div>
    </div>
  </div>
</div>


<div class="social-header hidden-sm hidden-xs">
  <div class="container">
    <div class="col-md-3 col-lg-3">
      <div class="folow-us">
      Follow us on :
        <a href="https://www.facebook.com/globalonlinetuitions" target="_blank"><img src="images/facebook.png" alt="social"/></a>
      </div>
    </div>
    <div class="col-md-9 col-lg-9 no-pad-menu">
      <div class="conatct-detail">
        <!--<div class="Parents-login">
        <a href="https://gaitview.com/gvt/login.html" target="_blank">Parents Login</a>
        </div>
        <div class="downloads">
        <a href="downloads.html">Downloads</a>
        </div>-->

        <div class="phone-num">
          <i class="fa fa-phone" aria-hidden="true"></i>
          +91 97157 53588, +91 86829 01099
        </div>
        <div class="emailaddress">
          <i class="fa fa-envelope" aria-hidden="true"></i>
          <a href="mailto:globalonlinetuitions@gmail.com">globalonlinetuitions@gmail.com</a>
        </div>
        <!--<div class="aff-no">
        Affiliation No : 12345
        </div>-->
      </div>
    </div>
  </div>
</div>

<div class="clearfix"></div>
<div class="menu-part">
  <div class="container">
    <div class="col-md-3 col-sm-3 col-xs-6 small-dev-logo">
      <div class="logo-gts">
        <a href="/"><img src="img/GOT_Logo_Opaque.png" alt="logo" class="img-responsive" style="height:92px;"/></a>
      </div>
    </div>
    <div class="col-md-9 col-sm-9 col-xs-12 no-pad-menu">
      <div class="menu">
        <div class="main_menu">
          <nav class="navbar navbar-inverse no-pad-sd menu-smal-dev">
            <div class="container-fluid no-pad-sd">
              <div class="navbar-header">
                <button type="button" class="navbar-toggle" data-toggle="collapse" data-target="#myNavbar"> <span class="icon-bar"></span> <span class="icon-bar"></span> <span class="icon-bar"></span> </button>
              </div>
              <div class="collapse navbar-collapse no-pad-sd" id="myNavbar">
                <ul class="nav navbar-nav">
                  <li class="active"><a href="/">HOME</a></li>
                  <li><a href="courses.php">Subjects</a></li>
                  <li><a href="career.php">Career</a></li>
                </ul>
              </div>
            </div>
          </nav>
        </div>
      </div>
    </div>
  </div>
</div>
</header>